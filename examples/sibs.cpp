...
int main(int argc, char **argv) {
    try {
        ...
        auto msg = xenon::parse(
            start, "220008342E1F7F61BA04C697D176821A7A9F4EA20663F3");
        ...
        if (c == msg.end())
            IT_PANIC("can't find SIB-Data-variable");

        // display the bits of the inner sib
        cout << c->name() << " " << c->bits << "\n";

        // now parse the inner sib as a SysInfoType19
        // We first get the record we are interested from the spec, and then use
        // it to parse.
