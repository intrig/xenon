  ...
  public:
    ...
    bool operator==(xv_field const &rhs) const {
        ...
        if (mask.value && value != rhs.value)
            return false;
        if (mask.length && length != rhs.length)
            return false;
        if (sbs != rhs.sbs)
            return false;
        if (mask.show_desc && desc != rhs.desc)
            return false;
        return true;
    }
...
class xv_message {
  public:
    ...
    bool compare(xv_message const &rhs) const {
        ...
        if (bs != rhs.bs) {
            std::cerr << "bitstrings don't match: " << ict::to_string(bs)
                      << " != " << ict::to_string(rhs.bs) << location(*this)
                      << "\n";
            return false;
        }

        if (fields != rhs.fields) {
            std::cerr << "fields don't match: " << xddl_file << " "
                      << ict::to_hex_string(bs.begin(), bs.end()) << '\n';
            return false;
        }
        return true;
    }

    bool empty() { return xddl_file.empty() && bs.empty() && fields.empty(); }
    template <typename Stream> void to_stream(Stream &os) const {
...
class xv_file {
  public:
    ...
    int validate() {
        ...
        if (!xddl_path.empty())
            doc.xddl_path.push_back(xddl_path);
        xenon::message m;
        std::string xddl_file;
        ...
        for (; i != data.end(); ++i) {
            if (xddl_file != i->xddl_file) {
                ...
                if (verbose)
                    std::cout << "processing xddl file " << xddl_file << "\n";
                doc.clear();
...
int main(int argc, char **argv) {
    try {
        ...
        line.add(ict::option("force-xv", 'f', "force printing in xv format",
                             [&] { force_print = true; }));
        line.add(ict::option("no-paths", 'p', "don't check paths",
                             [&] { mask.path = false; }));
        line.add(ict::option("no-lengths", 'L', "don't check lengths",
                             [&] { mask.length = false; }));
        line.add(ict::option("no-values", 'v', "don't check values",
                             [&] { mask.value = false; }));
        line.add(ict::option("no-desc", 'd', "don't check descriptions",
                             [&] { mask.show_desc = false; }));
        line.add(ict::option("verbose", 'V', "show progress",
                             [&] { verbose = true; }));
        line.add(ict::option("timing", 't', "show timing",
                             [&] { show_time = true; }));
        line.add(ict::option("xddl_path", 'X', "XDDL path", "",
                             [&](const std::string &v) { xddl_path = v; }));

        line.add_note("Arguments (processed in order):");
        line.add_note("  xddl file : XDDL file will be loaded and used for "
                      "subsequent decoding");
        line.add_note("  xv file : xv file will be validated (or redecoded and "
                      "printed if -p)");

        line.parse(argc, argv);

        if (line.targets.empty())
            IT_PANIC("no arguments given");
        ...
        if (!xddl_path.empty())
            spec2.xddl_path.push_back(xddl_path);

        for (auto const &arg : line.targets) {
            if (verbose)
                std::cout << "processing target " << arg << "\n";
        ...
        timer.stop();
        if (show_time)
            std::cout << "total time: " << ict::to_string(timer) << "\n";
