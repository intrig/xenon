...
ict::multivector<int> two() {
    ...
    for (int i = 40; i < 50; ++i) tree.root()[5].emplace_back(i);
    for (int i = 100; i < 103; ++ i) tree.root()[5][3].emplace_back(i);
    for (int i = 0; i < 10; ++i) tree.root()[5][7].emplace_back(i);
    ict::verify(tree);
    return tree;
...
bool grow(ict::multivector<int> & tree) {
    for (int i = 200; i < 240; ++i) tree.root()[5].emplace_back(i);
    ict::verify(tree);
    return true;
}
...
void iterate() {
    ...
    for (auto i = ict::ascending_begin(tree.root()[5][7]); !i.is_root(); ++i) std::cout << *i << "\n";

    std::cout << "\nagain, using is_root() instead of rend():\n";
    for (auto i = ict::ascending_begin(tree.root()[5][7]); !i.is_root(); ++i) std::cout << *i << "\n";
}
...
void constness() {
    ict::multivector<int> tree = two();
    std::cout << ict::to_debug_text(tree);
