...
class NamedNumberList : public ast {
  public:
    NamedNumberList(NamedNumber *number) : extension(false) { add(number); }
    ...
    void add(NamedNumber *number) {
        if (number->is_extension_marker) {
            ...
            if (extension)
                ext_items.push_back(number);
            else
                base_items.push_back(number);
        }
    }
...
class Type : public ast {
  public:
    ...
    virtual std::string instance() const {
        std::ostringstream os;
        os << "line " << line << ": undefined instance for type "
           << typeid(*this).name();
        IT_PANIC(os.str());
    }
    ...
    virtual std::string instance(Name *) const {
        std::ostringstream os;
        os << "line " << line << ": undefined name instance for type "
           << typeid(*this).name();
        IT_PANIC(os.str());
    }
    ...
    virtual std::string instance(Name *, SubtypeSpec *) const {
        std::ostringstream os;
        os << "line " << line << ": undefined subtype instance for type "
           << typeid(*this).name();
        IT_PANIC(os.str());
    }
    ...
    virtual std::string declaration(Name *) const {
        std::ostringstream os;
        os << "line " << line << ": undefined declaration for type "
           << typeid(*this).name();
        IT_PANIC(os.str());
    }
    ...
    virtual std::string declaration(Name *, SubtypeSpec *) const {
        std::ostringstream os;
        os << "line " << line << ": undefined subtype declaration for type "
           << typeid(*this).name();
        IT_PANIC(os.str());
    }
    ...
    virtual std::string value_assignment(Name *, Value *) const {
        std::ostringstream os;
        os << "line " << line << ": undefined value assignment for type "
           << typeid(*this).name();
        IT_PANIC(os.str());
    }
...
class DefinedType : public Type {
  public:
    ...
    virtual std::string instance(Name *id, SubtypeSpec *) const {
        // TODO: Ignore the subtype spec for now.  This only occurs for "WITH
        // COMPONENTS" and it is unclear if there is an encoding implications.
        return instance(id);
    }
...
class NamedType : public Type {
  public:
    ...
    virtual std::string instance() const {
        // if there is a type->def_value then call instance once with def_value
        // not set, to treat it like any ordinary optional value.  Then set it
        // to the type->def_value and call instance again to create itself as
        // just a visible property.
        std::ostringstream os;
...
class SequenceType : public BuiltinType {
  public:
    ...
    virtual std::string instance() const { return items->instance(); }
    virtual std::string instance(Name *) const;

    virtual std::string declaration(Name *id) const {
        std::ostringstream os;
        os << "<record id=''" << id->str() << "''>" << instance()
           << "</record>";
        return os.str();
    }
...
class ChoiceType : public BuiltinType {
  public:
    ...
    virtual std::string instance(Name *name) const {
        std::ostringstream os;
        os << "<record name=''" << name->str() << "''>" << instance()
           << "</record>";
        return os.str();
    }
...
class EnumeratedType : public BuiltinType {
  public:
    ...
    std::string base_enum_field(Name *id,
                                std::vector<NamedNumber *> items) const;
...
class BitStringType : public BuiltinType {
  public:
    ...
    virtual std::string instance(Name *name) const {
        std::ostringstream os;
        os << var_length()
           << "<!-- BitStringType instance(name) --> <field length=''length'' "
              "name=''"
           << name->str() << "''/>";
...
class NamedBitStringType : public BuiltinType {
  public:
    NamedBitStringType(NamedNumberList *number_list)
...
class OctetString : public Type {
  public:
    ...
    virtual std::string instance(Name *name) const {
        std::ostringstream os;
        os << var_length() << "<field length=''length * 8'' name=''"
           << name->str() << "''/>";
...
class TypeAssignment : public Assignment {
  public:
    ...
    virtual std::string str() const { return type->declaration(id); }
...
class ModuleBody : public ast {
  public:
    ...
    virtual std::string str() const { return items->str(); }
...
class ModuleList : public ast {
  public:
    ...
    ModuleList(ModuleDefinition *module) : comments(true) { add(module); }

    void add(ModuleDefinition *module) { modules.push_back(module); }
...
class SymbolList : public ast {
  public:
    SymbolList(Name *symbol) { add(symbol); }

    void add(Name *symbol) { items.push_back(symbol); }
