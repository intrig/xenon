...
string
ElementTypeList::optional_bitmap(std::vector<Type *> const &item_list) const {
...
string
ElementTypeList::extension_bitmap(std::vector<Type *> const &item_list) const {
    std::ostringstream xml;
    xml << "<enc>";
...
string ElementTypeList::instance() const {
    ...
    for (vector<Type *>::const_iterator it = items.begin(); it != items.end();
         ++it) {
        xml << (*it)->instance();
    }
    ...
    if (!ext_items.empty()) {
        ...
        for (vector<Type *>::const_iterator it = ext_items.begin();
             it != ext_items.end(); ++it) {
            xml << (*it)->instance();
        }
...
string SequenceOfSubtype::choice_sequence(Name *id) const {
    ostringstream os;
    os << spec->size_field() << "<repeat name=''" << id->str()
       << "'' num=''count''>" << type->instance() << "</repeat>";
    return os.str();
}
...
string SequenceOfSubtype::instance(Name *id) const {
    ...
    if (dynamic_cast<ChoiceType *>(type)) {
        ...
        os << spec->size_field();
        os << "<repeat name=''" << id->str() << "'' num=''count''>"
           << type->instance() << "</repeat>";
    }
    return os.str();
}
...
string ValueRange::size_str() const { return size_string(lower(), upper()); }
...
string IntegerType::declaration(Name *id, SubtypeSpec *spec) const {
    ostringstream os;
    os << "<record id=''" << id->str() << "''>" << instance(id, spec)
       << "</record>";
    return os.str();
}
...
ValueAssignment::ValueAssignment(Name *id, Type *type, Value *value)
    : identifier(id), type(type), value(value) {
    local_constants[id->str()] = value;
}

string EnumeratedType::instance() const { return instance(new Name("value")); }
...
string EnumeratedType::base_enum_field(Name *id,
                                       vector<NamedNumber *> items) const {
    ostringstream os;
    ...
    if (def_value) {
        ...
        os << "<field name=''" << id->str() << "'' length=''"
           << ict::required_bits(1, items.size()) << "''>";
...
string NamedBitStringType::declaration(Name *name, SubtypeSpec *spec) const {
    ostringstream os;
    os << "<record id=''" << name->str() << "''>" << instance(name, spec)
       << "</record>";
    return os.str();
}
...
string NamedBitStringType::instance(Name *id, SubtypeSpec *spec) const {
    ...
    vector<NamedNumber *>::iterator it;
    for (it = number_list->items.begin(); it != number_list->items.end();
         ++it) {
        // os <<"<field name=''" << (*it)->name() << "'' length=''" <<
        // (*it)->value->str() << "''/>";
...
string OctetString::declaration(Name *id, SubtypeSpec *spec) const {
    ostringstream os;
    os << "<record id=''" << id->str() << "''>"
       << instance(new Name("value"), spec) << "</record>";
    return os.str();
}
    ...
    if (ContainingSubtype *s = dynamic_cast<ContainingSubtype *>(spec)) {
        ...
        os << var_length()
           << "<field name=''octet size'' length=''8''/>"
              "<record name=''"
           << id->str() << "'' length=''{octet size} * 8'' href=''#"
           << s->size_str() << "''/>";
#else
        os << var_length() << "<record name=''" << id->str()
...
string SingleValue::size_str() const { return value->qstr(); }
...
string DefinedType::instance(Name *name) const {
    ...
    if (def_value) {
        // os << "<!-- DefinedType(name) default=''value(" <<
        // def_value->value->name() << ")'' -->";
        ...
        if (it == defined_types.end()) {
            if (!ignore_warnings)
                WARN("[line "
                     << type_name->line
                     << "] unresolved type reference: " << type_name->str());
        }
...
string DefinedType::instance() const {
    ...
    if (it == defined_types.end()) {
        if (!ignore_warnings)
            WARN("[line " << type_name->line << "] unresolved type reference: "
                          << type_name->str());
    }
...
bool SubtypeSpec::is_range() const { return lower()->str() != upper()->str(); }
