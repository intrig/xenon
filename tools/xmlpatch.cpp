...
class XmlPatch {
    ...
    XmlPatch(std::string filename) : filename(filename), state(New)
    {
        ...
        while (!is.eof())
        {
            ...
            else if (line == "old:") state = Old;
            else if (line == "new:") state = New;
            else if (!line.empty()) {
                switch (state)
                {
                    case Old:
    ...
    std::string filename;
    std::vector <std::pair <std::vector<std::string>, std::vector<std::string> > >  patches;
    State state, last_state;
};

typedef std::vector <std::pair <std::vector<std::string>, std::vector<std::string> > > Patches;
...
class XmlSource
{
    ...
    std::string patch(XmlPatch & patch)
    {
        ...
        for (patches_it = patch.patches.begin(); patches_it != patch.patches.end(); ++ patches_it)
        {
            bool done = false;
            auto it = lines.end();
            do {
                ...
                if (it==lines.end()) done = true;
                else {
                    *it = ""; 
                    lines.insert(it, patches_it->second.begin(), patches_it->second.end());
                }
            } while (!done);
        }
