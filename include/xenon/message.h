...
inline int64_t eval_variable(const std::string &name, spec::cursor context) {
    ...
    // Tell the message parser is another node that is dependent on the value of
    // nodes based on this element i.e., its not a plain old field.
...
// TODO: move this one to spec.h, otherwise loading a doc is dependent on
// message.h
inline int64_t
eval_function(const std::string &name, spec::cursor,
              const std::vector<ict::expression::param_type> &params) {
    if ((name == "sizeof") || (name == "defined") || (name == "value") ||
        (name == "gsm7")) {
...
inline int64_t eval_variable_list(const std::string &first,
                                  const std::string &second,
                                  message::cursor context) {
    auto f = get_variable(first, context);
    auto s = find_first(f, second);
    if (s == f.end())
        IT_PANIC("unmatched second variable: " << second);
...
template <typename S, typename C> inline void description_xml(S &os, C c) {
    auto d = c->elem->v->vdescription(c->elem, c);
    if (!d.empty())
        os << "<description>" << ict::xmlize(d) << "</description>";
}

namespace ht { // heading type
// TODO: these enums need to be a dynamic data structure.  Eventually supporting
// user added items.
enum index {
    name,
    row,
