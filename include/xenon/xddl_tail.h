...
template <> inline std::string name_of(const xenon::element &e) {
    return e.name();
}
template <typename T, typename U> inline T *get_ptr(const U &v) {
    return std::dynamic_pointer_cast<T>(v).get();
}

struct cmp_name {
    cmp_name(const std::string &name) : name(name) {}
    bool operator()(const element &e) { return e.name() == name; }
    std::string name;
};

inline std::string tag_of(const element &e) { return e.tag().c_str(); }
...
inline std::string description(spec::cursor self, spec::cursor referer,
                               msg_const_cursor c) {
    if (!c->desc.empty())
        return c->desc;
...
inline SpecCursor find_prop(SpecCursor start, const std::string &prop_name) {
    ...
    auto exp = std::find_if(xddl.begin(), xddl.end(), [&](const element &c) {
        return c.tag() == "export";
    });
    if (exp != xddl.end()) {
