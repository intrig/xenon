...
namespace lua {
struct lua_State;
// using this wrapper instead of just lua_State is required since lua_State is
// opaque.
...
inline lua::lua_State *get(std::shared_ptr<state_wrapper> const &l) {
    return l.get()->p;
}
} // namespace lua
...
template <typename Cursor>
inline ict::expression create_expression(Cursor start,
                                         const std::string &value) {
    ict::expression exp;
    if (value.empty())
        exp = ict::expression();
    else
        exp = ict::expression(value, start);
    return exp;
}
...
inline int64_t create_integer(Cursor, const std::string &s) {
    ...
    switch (s[0]) {
    case '#':
        if (ict::string_to_int64(v, s.c_str() + 1, 16))
            return v;
        break;
    case '@':
        if (ict::string_to_int64(v, s.c_str() + 1, 2))
            return v;
        break;
    default:
        if (ict::string_to_int64(v, s.c_str()))
            return v;
        break;
    }
...
template <typename AttList> inline size_t fragment_test(const AttList &) {
    return 0;
}

template <typename AttList> inline size_t record_test(const AttList &atts) {
    auto id = find_att(atts, "id");
    auto href = find_att(atts, "href");
    if (!id.empty() && !href.empty())
        IT_PANIC("both id and href are defined");
    if (!href.empty())
        return 1;
    return 0;
}

template <typename AttList> inline size_t repeat_test(const AttList &atts) {
    int ret = 0;
    ...
    if (num && (min || max))
        IT_PANIC(
            "<repeat> with num attribute cannot have min or max attribute");
    if (num)
        ret = 1;
    if (max || min)
        ret = 2;
