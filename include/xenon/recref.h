...
namespace xenon {
template <typename Rec> void parse_recref(std::string const &ref, Rec &rec) {
    if (ref[0] == '#') {
        if (ref.size() == 1)
            return;
        rec.anchor = ref;
        ...
        if (j != std::string::npos)
            rec.file = ref.substr(i, j - i);
        else
            rec.file = ref.substr(i);

        if (j != std::string::npos)
            rec.anchor = ref.substr(j);
...
struct recref {
    ...
    void str(ict::osstream &os) const { os << path << file << anchor; }
    ...
    bool friend operator==(const recref &a, const recref &b) {
        return (a.path == b.path) && (a.file == b.file) &&
               (a.anchor == b.anchor);
    }
