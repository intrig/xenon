...
struct node {
    ...
    enum node_flags { visible, flag_count };
    ...
    bool is_pof() const {
        return is_field() && !elem->flags.test(element::dependent_flag);
    } // "plain ol' field"
    bool is_visible() const { return flags.test(visible); }
    void set_visible(bool yes) { flags.set(visible, yes); }

    void set_incomplete() { type = incomplete_node; }
    ...
    friend bool operator==(const node &a, int64_t b) { return a.value() == b; }
...
struct node_info_type {
    ...
    node_info_type(const char *name, flag_type flags)
        : name(name), flags(flags) {}
    node_info_type(const char *name, flag_type flags, start_tag_op op)
        : name(name), flags(flags), start_tag(op) {}
    const char *name;
    flag_type flags;
    start_tag_op start_tag = [&](ict::osstream &, msg_const_cursor) {};
};

inline std::string node_tag(const node &n) { return n.tag().c_str(); }
...
template <typename Stream> Stream &to_debug(Stream &os, const node &n) {
    os << n.tag() << " " << n.mnemonic() << " " << n.name() << " ("
       << (n.is_field() ? "field " : "") << (n.is_prop() ? "prop " : "")
       << (n.is_extra() ? "extra " : "") << (n.consumes() ? "consumes " : "")
       << (n.is_terminal() ? "term " : "")
       << (n.is_encoding() ? "encoding " : "") << (n.is_oob() ? "oob " : "")
       << (n.is_pof() ? "pof " : "") << ')';
    return os;
}

template <> inline std::string name_of(const xenon::node &n) {
    return n.name();
}
} // namespace xenon
