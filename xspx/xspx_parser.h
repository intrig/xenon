...
struct xml_att_type {
    ...
    bool required = false;
    bool hidden =
        false; // TODO: hidden from docs from now but could still be used
    std::string def;
...
inline std::ostream &to_code(std::ostream &os, const xml_att_type &att,
...
inline std::string to_param_list(const xml_att_list &atts,
                                 const custom_type_list &custom_types) {
    ...
    for (const auto &att : atts) {
        if (att.local) {
            std::ostringstream s;
            auto ct =
                xenon::find_by_name(custom_types, std::string(att.type_name));
...
inline std::string to_init_list(const elem_type &elem) {
    ...
    // ") << ")";
    if (!params.empty())
        s << ict::join(params.begin(), params.end(), ", ");
    return s.str();
}
...
enum type { header_decl, header_impl, source_impl };
}
...
class xsp_parser {
  public:
    ...
    void open(const std::string &filename) { p.open(filename); }
    ...
    std::ostream &to_decl(std::ostream &os, const elem_type &elem,
                          const std::string &root) const;
    std::ostream &to_choice_init(std::ostream &os,
                                 const choice_type &choice) const;
    std::ostream &start_handler_contents(std::ostream &os,
                                         const elem_type &elem) const;
    void att_param(std::ostream &os, const std::string &start,
    ...
    for (auto &x : uniqs) {
        if (!x.is_mod) {
            ...
            os << "case ict::" << x.name << "_uid : " << name
               << "(c, static_cast<ict::" << x.name
               << "*>(c->v.get()), std::forward<Args>(args)...); ";
            os << "break;\n";
        }
    }
...
template <typename Cont> std::string pop_last(Cont &cont) {
    if (cont.empty())
        return "";
