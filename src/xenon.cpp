...
const int DateTime::daysmonth[13] = {0,  31, 28, 31, 30, 31, 30,
                                     31, 31, 30, 31, 30, 31};
const int DateTime::daysmonthleap[13] = {0,  31, 29, 31, 30, 31, 30,
                                         31, 31, 30, 31, 30, 31};

template <typename State> inline message::cursor get_cursor(State *L) {
...
static int script_Description(lua::lua_State *L) {
    ...
    if (c.is_root())
        ...
        lua::lua_pushstring(L, description(c).c_str());
    return 1;
}
...
static int script_Gsm7(lua::lua_State *L) {
    ...
    if (s) {
        ...
        if (!c.is_root())
            fill = ict::to_integer<int>(c->bits);
    }
...
void type::vto_string(std::ostream &os) const {
    os << id << '\n';
    for (auto &i : items)
        os << "  item: " << i.first << " " << i.second.value << '\n';
    for (auto &i : ranges)
        os << "  range: " << i.second.value << '\n';
}
...
void xcase::vend_handler(spec::cursor, spec &parser) {
    ...
    if (xs->has_default)
        IT_PANIC("<default> must be the last case");
...
void field::vend_handler(spec::cursor self, spec &parser) {
    if (has_anon_type(self)) {
        ...
        href = "anon";
        auto e =
            self.emplace(create_type_struct("anon", mv.root()), "type", "anon");
        e->parser = &parser;
        e->line = self->line;
    }
}
...
void script::vend_handler(spec::cursor, spec &dom) {
    ...
    if (lua::luaL_loadstring(p, dom.cdata.c_str()))
        IT_PANIC(lua::lua_tostring(p, -1));
    ...
    ict::recurse(parent, [&](spec::cursor &i) {
        if (i->tag() == tag) {
            if (auto f = get_ptr<T>(i->v)) {
                ...
                if (c == m.end())
                    m[f->id] = i;
                else if (f->id != anon && !f->id.empty())
                    IT_PANIC("id " << f->id << " already used");
            }
        }
    });
}
...
void link_ref(Cursor &self, recref &url, Map &m) {
    ...
    if (url.is_local()) {
        auto i = m.find(url);
        if (i == m.end())
            IT_PANIC("cannot find local reference: " << url);
        else
            self->v->vset_ref(i->second);
    } else
        self->v->vset_ref(self.end());
}
...
void link_type_ref(Cursor &self, T first, T last, Map &recmap) {
    while (first != last) {
        if (first->second.href.is_local()) {
            ...
            if (i == recmap.end())
                IT_PANIC("cannot find local reference: " << first->second.href);
            first->second.ref = i->second;
        } else
            first->second.ref = self.end();
        ++first;
    }
}
...
void link_local_refs(Cursor parent, RecMap &rec_map, TypeMap &type_map) {
    ict::recurse(parent, [&](spec::cursor self) {
        ...
        if (!url.empty()) {
            if (self->tag() == "fragment" || self->tag() == "record")
                link_ref(self, url, rec_map);
            else if (self->uid == field_uid || self->tag() == "prop" ||
                     self->tag() == "setprop")
                link_ref(self, url, type_map);
        }

        else if (self->tag() == "type")
            link_type_refs(self, rec_map);
    });
}
...
void link_reflective_properties(spec::cursor doc_root) {
    ...
    ict::recurse(doc_root, [&](spec::cursor self) {
        ...
        if (e.uid == field_uid || e.tag() == "prop" || e.tag() == "setprop") {
            if (globs.find(e.name()) != globs.end())
                e.flags.set(element::global_flag);
        }
    });
}

// set a flag of all descendents
template <typename Cursor, typename T> void set_flag(Cursor parent, T flag) {
    recurse(parent, [&](spec::cursor self) { self->flags.set(flag); });
}
...
inline void parse_children(spec::cursor self, message::cursor parent,
                           ict::ibitstream &bs) {
    if (parent.empty())
        parent.reserve(self.size());
    ict::bitmarker mk{bs};
...
void element::var_type::vparse(spec::cursor self, message::cursor parent,
                               ict::ibitstream &bs) const {
    parse_children(self, parent, bs);
}

void xddl::vparse(spec::cursor self, message::cursor parent,
                  ict::ibitstream &bs) const {
    auto st = find_child_with_tag(self, "start");
    if (st == self.end())
        parse_children(self, parent, bs);
...
void start::vparse(spec::cursor self, message::cursor parent,
                   ict::ibitstream &bs) const {
    parse_children(self, parent, bs);
...
void parse_ref(spec::cursor self, message::cursor parent, ict::ibitstream &bs,
               spec::cursor &ref, const recref &href, Parser parser) {
    try {
        if (ref == self.end()) {
...
void fragment::vparse(spec::cursor self, message::cursor parent,
                      ict::ibitstream &bs) const {
    parse_ref(self, parent, bs, ref, href, self->parser);
}

void jump::vparse(spec::cursor self, message::cursor parent,
                  ict::ibitstream &bs) const {
    try {
        // get the field this jump is based on
        auto c = rfind_first(leaf(parent), base); // c is a field node
        if (c.is_root())
            IT_PANIC("cannot find " << base);
        ...
        if (!f)
            IT_PANIC(base << " is not a field");

        if (f->ref == elem.end()) {
            auto url =
                relative_url(elem->parser->file, f->href); // create an abs url.
...
void reclink::vparse(spec::cursor self, message::cursor parent,
                     ict::ibitstream &bs) const {
...
void record::vparse(spec::cursor self, message::cursor parent,
                    ict::ibitstream &bs) const {
    auto rec = parent.emplace(node::record_node, self);
    ict::constraint ct(bs, length.value(rec));
...
void pad::vparse(spec::cursor self, message::cursor parent,
                 ict::ibitstream &bs) const {
    size_t l = mod - ((bs.tellg() - bs.last_mark() - offset) % mod);
    if (l > 0 && l < mod) {
        l = std::min(l, bs.remaining());
        if (l)
            parent.emplace_back(node::field_node, self, bs.read(l));
    }
}

void peek::vparse(spec::cursor self, message::cursor parent,
                  ict::ibitstream &bs) const {
...
void prop::vparse(spec::cursor self, message::cursor parent,
                  ict::ibitstream &) const {
...
void setprop::vparse(spec::cursor self, message::cursor parent,
                     ict::ibitstream &) const {
    auto v = value.value(leaf(parent));
...
void cstr::vparse(spec::cursor self, message::cursor parent,
                  ict::ibitstream &bs) const {
...
void xif::vparse(spec::cursor self, message::cursor parent,
                 ict::ibitstream &bs) const {
    if (expr.value(leaf(parent)) != 0)
        parse_children(self, parent, bs);
}

inline message::cursor add_repeat_record(spec::cursor self,
                                         message::cursor parent,
                                         ict::ibitstream &bs) {
...
void num_repeat::vparse(spec::cursor self, message::cursor parent,
                        ict::ibitstream &bs) const {
    auto num_value = num.value(leaf(parent));
    if (num_value > 0) {
        auto rep = parent.emplace(node::repeat_node, self);
        rep.reserve(num_value);
        for (int64_t i = 0; i < num_value; ++i)
            add_repeat_record(self, rep, bs);
    }
}

void bound_repeat::vparse(spec::cursor self, message::cursor parent,
                          ict::ibitstream &bs) const {
...
void xswitch::vparse(spec::cursor self, message::cursor parent,
                     ict::ibitstream &bs) const {
    ...
    if (i != cases.end()) {
        ...
        parse_children(self.end() - 1, parent, bs);
}

void xwhile::vparse(spec::cursor self, message::cursor parent,
                    ict::ibitstream &bs) const {
    auto rec = parent.emplace(node::record_node, self);
...
std::string type::value(spec::cursor x, message::const_cursor c) const {
    ...
    if (s) {
        ...
        IT_WARN("error: " << x->parser->file << ":" << x->line << " "
                          << lua::lua_tostring(p, -1));
...
std::string type::vdescription(spec::cursor referer,
                               message::const_cursor c) const {
    if (l)
        return value(referer, c);
    return enum_string(referer, c);
}

template <typename T>
std::string get_description(const T *self_ptr, spec::cursor self,
                            message::const_cursor c) {
    if (!c->desc.empty())
        return c->desc;
    if (self_ptr->href.empty())
        return "";
    else if (self_ptr->ref == self.end()) {
        try {
            auto url = relative_url(self->parser->file,
                                    self_ptr->href); // create an abs file path.
...
std::string field::vdescription(spec::cursor self,
                                message::const_cursor c) const {
    return get_description(this, self, c);
}

std::string field::venum_string(spec::cursor self,
                                message::const_cursor c) const {
    if (ref == self.end())
        return "";
    else
        return ref->v->venum_string(self, c);
}

std::string prop::vdescription(spec::cursor self,
                               message::const_cursor c) const {
    return get_description(this, self, c);
}

std::string prop::venum_string(spec::cursor self,
                               message::const_cursor c) const {
    if (ref == self.end())
        return "";
    else
...
std::string setprop::vdescription(spec::cursor self,
                                  message::const_cursor c) const {
    return get_description(this, self, c);
}

std::string setprop::venum_string(spec::cursor self,
                                  message::const_cursor c) const {
    if (ref == self.end())
        return "";
    else
...
std::ostream &operator<<(std::ostream &os, const node &n) {
    ...
    os << n.name() << " " << n.bits.bit_size() << " " << v << " " << std::hex
       << v << " (" << n.mnemonic() << ")" << std::dec;
    return os;
}
